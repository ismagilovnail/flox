...
**Статус:** ВСЕ ТРИ ПРАВКИ ПРИМЕНЕНЫ в спеку 2026-08-13.
A1 и A2 → §45, §59, §80, CLAUDE.md инвариант #3 и NEVER.
A3 → §38, §39, §58, CLAUDE.md ROUTING QUICK REF.
Документ сохранён как обоснование принятых решений — вносить повторно нечего.
**Дата:** 2026-08-13
...
| № | Правка | Раздел | Тип | Статус | Риск невнесения |
|---|---|---|---|---|---|
| A1 | Ключ дедупа — трёхчастный, с явной ролью txn id | §45 + CLAUDE.md #3 + §59 | разночтение | ✅ применена | Потерянные повторные депозиты **или** задвоенные инвойсы |
| A2 | Запрет отката статуса конверсии назад в HOLD | §45 (новый блок) + §59 | пробел | ✅ применена | Ночные реплеи партнёров вынимают выручку из закрытых отчётов |
| A3 | Weighted pick детерминирован по ключу визита, а не по ГСЧ | §38 + §39 + §58 | уточнение | ✅ применена | Реплики трекера расходятся в решениях; A/B-данные шумят |
